module JsonErrorsHandler
  ...
  class ZombieDriverServerError < StandardError
  ...
  included do
    ...
    rescue_from ZombieDriverServerError, with: lambda { |exception| zombie_driver_server_error(exception.error_message) }
  end

  protected

  def zombie_driver_server_error(error)
    title = 'Zombie Driver is not responding'
